<x-layouts.app title="Transaksi Jurnal">
  <x-slot:scripts>
    <script>
      ...
      const fetchAllNilaiByTransaksiJurnalId = async (id) => {
        try {
          ...
          nilai = responseJson.data.nilai_jurnal;
        } catch (error) {
      ...
      const updateNilai = async (data) => {
        try {
          ...
          const response = await fetch(`/transaksi-jurnal/${card.dataset.id}`, {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json',
              'X-CSRF-TOKEN': csrfToken,
          ...
          fetchAllNilaiByTransaksiJurnalId(card.dataset.id);
        }
      }
  ...
  <x-card id="card" data-id="{{ $transaksiJurnal->id }}">
      ...
      <form action="{{ route('transaksi-jurnal.update', $transaksiJurnal->id) }}" method="POST"
        ...
        <div class="w-full lg:w-1/4 px-4 space-y-4">
          <div class="space-y-2">
            ...
            <x-forms.text-input id="kwitansi" name="kwitansi" :value="old('kwitansi', $transaksiJurnal->kwitansi)" placeholder="Masukan Kwitansi"
          ...
          <div class="space-y-2">
            ...
            <x-forms.text-input type="date" id="tanggal" name="tanggal" :value="old('tanggal', $transaksiJurnal->tanggal)"
          ...
          <div class="space-y-2">
            ...
            <x-forms.textarea id="deskripsi" name="deskripsi" placeholder="Masukan Deskripsi" :error="$errors->first('deskripsi')"
              required>{{ old('deskripsi', $transaksiJurnal->deskripsi) }}</x-admin.forms.textarea>
          ...
          <div class="space-y-2">
            ...
            <x-forms.textarea id="keterangan_jurnal" name="keterangan_jurnal" placeholder="Masukan Keterangan Jurnal"
              :error="$errors->first('keterangan_jurnal')"
              required>{{ old('keterangan_jurnal', $transaksiJurnal->keterangan_jurnal) }}</x-admin.forms.textarea>
