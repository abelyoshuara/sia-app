<x-layouts.app title="Transaksi Penyesuaian">
  ...
  <div class="space-y-1">
    ...
    <p class="text-sm text-gray-600 dark:text-neutral-400">
      Form ubah transaksi penyesuaian.
    </p>
  </div>

  <x-card id="card" data-id="{{ $transaksiPenyesuaian->id }}">
      ...
      <form action="{{ route('transaksi-penyesuaian.update', $transaksiPenyesuaian->id) }}" method="POST"
        ...
        <div class="w-full lg:w-1/4 px-4 space-y-4">
          <div class="space-y-2">
            ...
            <x-forms.text-input type="date" id="tanggal" name="tanggal" :value="old('tanggal', $transaksiPenyesuaian->tanggal)"
          ...
          <div class="space-y-2">
            <x-forms.input-label for="nilai" :value="__('Nilai yang disesuaikan')" />
            <x-forms.text-input type="number" id="nilai" name="nilai" :value="old('nilai', $transaksiPenyesuaian->nilai)" onkeyup="countJumlah()"
              placeholder="Masukan Nilai" :error="$errors->first('nilai')" required />
            <x-forms.input-error :message="$errors->first('nilai')" />
          </div>

          <div class="space-y-2">
            <x-forms.input-label for="waktu" :value="__('Waktu yang disesuaikan')" />
            <x-forms.text-input type="number" id="waktu" name="waktu" :value="old('waktu', $transaksiPenyesuaian->waktu)" onkeyup="countJumlah()"
              placeholder="Masukan Waktu" :error="$errors->first('waktu')" required />
            <x-forms.input-error :message="$errors->first('waktu')" />
          </div>

          <div class="space-y-2">
            <x-forms.input-label for="jumlah" :value="__('Jumlah')" />
            <x-forms.text-input id="jumlah" name="jumlah" :value="old('jumlah', $transaksiPenyesuaian->jumlah)" :error="$errors->first('jumlah')"
              placeholder="Hasil penjumlahan" readonly />
            <x-forms.input-error :message="$errors->first('jumlah')" />
          </div>
        </div>
        ...
        <div class="px-4 flex justify-end gap-x-2">
          <x-button type="submit">Simpan</x-admin.button>
            <x-button as="a" href="{{ route('transaksi-penyesuaian.index') }}" variant="white"
          ...
          <div
            ...
            <form action="" method="POST">
              ...
              <div class="flex justify-end items-center gap-x-2 py-3 px-4 border-t dark:border-neutral-700">
                <x-button type="button" variant="white" color="dark"
                  data-hs-overlay="#modal-nilai">Batal</x-admin.button>
                  <x-button type="submit" data-hs-overlay="#modal-nilai">Tambah</x-admin.button>
              </div>
            </form>
          </div>
        </div>
