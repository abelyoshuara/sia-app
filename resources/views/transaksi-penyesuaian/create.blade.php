<x-layouts.app title="Transaksi Penyesuaian">
  <x-slot:scripts>
    <script>
      ...
      let nilai = JSON.parse(localStorage.getItem('TRANSAKSI_PENYESUAIAN') || '[]');
      ...
      const addNilai = (values) => {
        // nilai = [...nilai, values];
        nilai.push(values);
        localStorage.setItem('TRANSAKSI_PENYESUAIAN', JSON.stringify(nilai));
      };
      ...
      const insertNilai = async (data) => {
        try {
          ...
          const response = await fetch('/transaksi-penyesuaian', {
            method: 'POST',
            headers: {
          ...
          localStorage.removeItem('TRANSAKSI_PENYESUAIAN');
          nilai = [];
          formTransaksi.reset();
          renderAllNilai();
        }
      ...
      <form action="{{ route('transaksi-penyesuaian.store') }}" method="POST"
        ...
        <div class="w-full lg:w-1/4 px-4 space-y-4">
          ...
          <div class="space-y-2">
            ...
            <x-forms.text-input type="number" id="nilai" name="nilai" :value="old('nilai')" onkeyup="countJumlah()"
          ...
          <div class="space-y-2">
            ...
            <x-forms.text-input type="number" id="waktu" name="waktu" :value="old('waktu')" onkeyup="countJumlah()"
          ...
          <div class="space-y-2">
            ...
            <x-forms.text-input id="jumlah" name="jumlah" :value="old('jumlah')" :error="$errors->first('jumlah')"
