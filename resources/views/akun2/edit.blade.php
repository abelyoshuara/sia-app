    ...
    <x-admin.card.content>
      <form action="{{ route('akun2.update', $akun2->id) }}" method="POST">
        ...
        <div class="space-y-4">
          ...
          <div class="space-y-2 @error('akun1s_id') error @enderror">
            <x-admin.forms.input-label for="akun1s_id"
              class="inline-block text-sm font-medium text-gray-800 dark:text-neutral-200" :value="__('Nama Akun 1')" />
            <x-admin.forms.advanced-select id="akun1s_id" name="akun1s_id">
              <option value="">Choose</option>
              @foreach ($akun1s as $akun1)
                <option value="{{ $akun1->id }}" @selected(old('akun1s_id', $akun2->akun1s_id) == $akun1->id)>{{ $akun1->nama }}</option>
              @endforeach
            </x-admin.forms.advanced-select>
            <x-admin.forms.input-error class="hidden hs-error:block" :message="$errors->first('akun1s_id')" />
          </div>
